    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != NULL) {
        ...
        if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
          ImGui_ImplVulkan_SetupRenderState(draw_data, command_buffer, rb,
                                            fb_width, fb_height);
        else
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer) {
  ...
  // Create the Upload Buffer:
  {
    ...
    buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    err =
        vkCreateBuffer(v->Device, &buffer_info, v->Allocator, &g_UploadBuffer);
    check_vk_result(err);
    VkMemoryRequirements req;
    ...
    check_vk_result(err);
    err =
        vkBindBufferMemory(v->Device, g_UploadBuffer, g_UploadBufferMemory, 0);
    check_vk_result(err);
  }
  ...
  {
    char* map = NULL;
    err = vkMapMemory(v->Device, g_UploadBufferMemory, 0, upload_size, 0,
                      (void**)(&map));
    check_vk_result(err);
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  // Create The Shader Modules:
  {
    ...
    err =
        vkCreateShaderModule(v->Device, &vert_info, v->Allocator, &vert_module);
    check_vk_result(err);
    ...
    err =
        vkCreateShaderModule(v->Device, &frag_info, v->Allocator, &frag_module);
    check_vk_result(err);
  }
  ...
  err = vkCreateGraphicsPipelines(v->Device, v->PipelineCache, 1, &info,
                                  v->Allocator, &g_Pipeline);
  check_vk_result(err);
...
void ImGui_ImplVulkan_DestroyDeviceObjects() {
  ...
  if (g_DescriptorSetLayout) {
    vkDestroyDescriptorSetLayout(v->Device, g_DescriptorSetLayout,
                                 v->Allocator);
...
void ImGui_ImplVulkan_Shutdown() { ImGui_ImplVulkan_DestroyDeviceObjects(); }
